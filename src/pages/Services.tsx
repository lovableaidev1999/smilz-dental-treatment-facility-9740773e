...
const serviceIcons: Record<string, string> = {
  "dental-implants": "🦷", "root-canal": "🔬", "orthodontics": "😬",
  "smile-designing": "✨", "tooth-whitening": "🌟", "scaling-polishing": "🪥",
};
  ...
  return (
    <>
      <SEOHead
        ...
        description={`Comprehensive dental treatments in Kolkata - implants, root canal, orthodontics, smile designing, teeth whitening, and more at ${settings?.general?.clinic_name ?? "Smilz Dental Treatment Facility"}.`}
      ...
      <section className="section-padding">
        <div className="container-narrow mx-auto">
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{[...Array(6)].map((_, i) => <div key={i} className="bg-card rounded-2xl p-8 shadow-card animate-pulse h-56" />)}</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {(services ?? []).map((service, i) => (
                <motion.div key={service.slug} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1, duration: 0.5 }}>
                  <Link to={`/services/${service.slug}`} className="group block bg-card rounded-2xl p-8 shadow-card hover:shadow-hover transition-all duration-300 h-full border border-border hover:border-primary/30">
                    <div className="text-5xl mb-5">{serviceIcons[service.slug] ?? "🦷"}</div>
                    <h2 className="text-xl font-heading font-bold text-foreground group-hover:text-primary transition-colors mb-3">{service.title}</h2>
